...
graph TD
    ...
    B --> E[Coding Agent<br/>15 tools]
    C --> F[Investigation Agent<br/>30+ tools]
    D --> G[Shared Tools<br/>llm_call, web_search]
```
...
```mermaid
graph TD
    A[Organization Defaults] --> B[Group Config<br/>optional]
    B --> C[Team Config]
```
...
```mermaid
graph LR
    A[Trigger<br/>Slack, GitHub] --> B[Agent Runtime]
    B --> C[Tools Execute]
    B --> D[Config Service]
    C --> E[Data Sources<br/>Coralogix, Snowflake, AWS]
```
